...
class ngsimDataset(Dataset):
    ...
    def __init__(self, mat_file, t_h=30, t_f=50, d_s=2, enc_size = 64, grid_size = (13,3),in_test=False):
    ...
    def __getitem__(self, idx):
        ...
        t = self.D[idx, 2]
        # remember we use different now_time in train and test in gan
        # t = self.D[idx, 2]+self.t_h
        grid = self.D[idx,8:]
        neighbors = []
        if not self.in_test:
            if dsId<=4 or dsId>=6:
        ...
        if not self.in_test:
            if dsId<=4 or dsId>=7:
                hero_max = hero_max*0.3048
        else:
            if 'nei' in self.mat_file:
                hero_max = hero_max*0.3048
    ...
    def getHistory(self,vehId,t,refVehId,dsId):
        if vehId == 0:
            ...
            if (dsId==6 and not self.in_test) or ('inter' in self.mat_file and self.in_test):
    ...
    def collate_fn(self, samples):
        ...
        return hist_batch, nbrs_batch, fut_batch, op_mask_batch,scale_batch,index_division
