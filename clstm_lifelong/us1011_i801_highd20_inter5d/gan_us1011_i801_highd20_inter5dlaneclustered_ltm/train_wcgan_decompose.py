...
# gen_set_name = None
# args_g['neigh_num'] = 6
real_set_name = 'data/TrainSet-inter5d-gan-lane-clustered.mat'
...
net = highwayNet(args)
net_g_us1011_i801_highd20p = highwayNet_g_compose(args_g_trained)
net_g_inter5d = highwayNet_g_compose(args_g_trained)

# we can start from last saved models
net_g_us1011_i801_highd20p.load_state_dict(torch.load('trained_models/generator_us1011_i801_highd20p_nsl_epoch702.tar'))
net_g_inter5d.load_state_dict(torch.load('trained_models/generator_inter5dlane_clustered_epoch102.tar'))

if args_d['use_cuda']:
    net_d = net_d.cuda()
    net_g = net_g.cuda()
    net = net.cuda()
    net_g_us1011_i801_highd20p = net_g_us1011_i801_highd20p.cuda()
    net_g_inter5d = net_g_inter5d.cuda()
...
for start_epoch in range(start_epoch,trainEpochs):
    ...
    for i, data in enumerate(trDataloader):
        ...
        with torch.no_grad():
            ...
            scene_us1011_i801_highd20p = net_g_us1011_i801_highd20p(pos_condition,index_division)
            scene_inter5d = net_g_inter5d(pos_condition,index_division)
            real_scene = torch.cat((scene_us1011_i801_highd20p,scene_inter5d),axis=0)
    ...
    if start_epoch%3==0 and start_epoch<trainEpochs:
        ...
        torch.save(net_g.state_dict(),
                   'trained_models/generator_us1011_i801_highd20p_inter5d_epoch' + str(start_epoch) + '.tar')
...
torch.save(net_g.state_dict(), 'trained_models/generator_us1011_i801_highd20p_inter5d.tar')
torch.save(net_d.state_dict(), 'trained_models/discriminator_us1011_i801_highd20p_inter5d.tar')
torch.save(net.state_dict(), 'trained_models/regression_us1011_i801_highd20p_inter5d.tar')
