...
trSet = ngsimDataset(mat_file=real_set_name,batch_size=args_d['batch_size'],local_normalise=1)
...
for start_epoch in range(start_epoch,trainEpochs):
    ...
    for i, data in enumerate(trDataloader):
        ...
        index_division_cp_classify_real = copy.deepcopy(index_division)
        index_division_cp_classify_fake = copy.deepcopy(index_division)
        index_division_cp_reg_real = copy.deepcopy(index_division)
        index_division_cp_reg_fake = copy.deepcopy(index_division)
        index_division_cp_gen_creal = copy.deepcopy(index_division)
        index_division_cp_gen_cfake = copy.deepcopy(index_division)
        index_division_cp_gen_rreal = copy.deepcopy(index_division)
        index_division_cp_gen_rfake = copy.deepcopy(index_division)
        index_division_plot = copy.deepcopy(index_division)
        ...
        if i%n_critic==0:
            ...
            recon_data = net_g(pos_condition,index_division)
        ...
        if i%sample_interval==0:
            ...
            for draw in range(draw_num):
                ...
                for j in current_id:
                    ...
                    plt.text(sample[j,0,-1],sample[j,1,-1],str(pos_condition[j,:].detach().cpu().numpy()))
    ...
    if start_epoch%3==0 and start_epoch<trainEpochs:
        ...
        torch.save(net_g.state_dict(),
                   'trained_models/generator_inter5dlane_clustered_epoch' + str(start_epoch) + '.tar')
...
torch.save(net_g.state_dict(), 'trained_models/generator_inter5dlane_clustered.tar')
torch.save(net_d.state_dict(), 'trained_models/discriminator_inter5dlane_clustered.tar')
torch.save(net.state_dict(), 'trained_models/regression_inter5dlane_clustered.tar')
