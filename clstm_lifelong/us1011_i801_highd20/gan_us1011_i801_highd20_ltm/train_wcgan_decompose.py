...
net = highwayNet(args)
net_g_us1011_i801 = highwayNet_g_compose(args_g_trained)
net_g_highd20p = highwayNet_g_compose(args_g_trained)

# we can start from last saved models
net_g_us1011_i801.load_state_dict(torch.load('trained_models/generator_us1011_i801_nsl_epoch402.tar'))
net_g_highd20p.load_state_dict(torch.load('trained_models/generator_highd20p_nsl_epoch801.tar'))

if args_d['use_cuda']:
    net_d = net_d.cuda()
    net_g = net_g.cuda()
    net = net.cuda()
    net_g_us1011_i801 = net_g_us1011_i801.cuda()
    net_g_highd20p = net_g_highd20p.cuda()
...
for start_epoch in range(start_epoch,trainEpochs):
    ...
    for i, data in enumerate(trDataloader):
        ...
        with torch.no_grad():
            ...
            scene_us1011_i801 = net_g_us1011_i801(pos_condition,index_division)
            scene_highd20p = net_g_highd20p(pos_condition,index_division)
            real_scene = torch.cat((scene_us1011_i801,scene_highd20p),axis=0)
    ...
    if start_epoch%3==0 and start_epoch<trainEpochs:
        ...
        torch.save(net_g.state_dict(),
                   'trained_models/generator_us1011_i801_highd20p_nsl_epoch' + str(start_epoch) + '.tar')
...
torch.save(net_g.state_dict(), 'trained_models/generator_us1011_i801_highd20p_nsl.tar')
torch.save(net_d.state_dict(), 'trained_models/discriminator_us1011_i801_highd20p_nsl.tar')
torch.save(net.state_dict(), 'trained_models/regression_us1011_i801_highd20p_nsl.tar')
